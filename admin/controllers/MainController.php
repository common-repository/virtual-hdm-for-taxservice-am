    ...
    if (class_exists('WC_Payment_Gateway')) {
        class MainController extends WC_Payment_Gateway
        {
            ...
            public function showTemplate($list)
            {
                ...
                $taxServiceUploadFilePath = get_option('hkd_tax_service_upload_file_path');
            ...
            public function saveTaxServiceSettings($request)
            {
                ...
                if (isset($_FILES['attach']) && !empty($_FILES["attach"]["name"])) {
                    ...
                    $savePath = $upload['baseurl'] . $directory_path;
                    $permissions = 0777;
                    $oldmask = umask(0);
                    ...
                    $savePath = $savePath . '/' . $files['name'];
                    update_option('hkd_tax_service_upload_file_path', $savePath);
                    $newFile = true;
                } else {
                    $savePath = get_option('hkd_tax_service_upload_file_path');
                }
                ...
                if ((!$taxServiceApiActivated && $savePath) || $newFile) {
                    $body = array(
                        'crn' => $taxServiceRegisterNumber,
                        'tpin' => $taxServiceTin,
                    );
                    $response = wp_remote_post($this->taxApiUrl . '/activate', array(
                        'sslverify' => false,
                        'sslcertificates' => $savePath,
                        'headers' => ['Content-Type' => 'application/json; charset=utf-8'],
                        'body' => json_encode($body, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
                    ));

                    if (is_wp_error($response)) {
                        ErrorService::hkdTaxServiceInsertTaxErrorReport(0, 'Tax service activate request Error', $response->get_error_message(), 'no');
                        update_option('hkd_tax_service_api_activated', 0);
                    } else {
                        $body = json_decode($response['body']);
                        if ($body->code == 0) {
                            $this->configureDepartments($taxServiceRegisterNumber, $savePath);
                            update_option('hkd_tax_service_api_activated', 1);
                        } else {
                            ErrorService::hkdTaxServiceInsertTaxErrorReport(0, 'Tax service activate request response Error', $body->error, 'no');
                            update_option('hkd_tax_service_api_activated', 0);
                        }
                    }
                }else{
            ...
            public function configureDepartments($taxServiceRegisterNumber, $savePath)
            {
                $departments = [
                    "crn" => $taxServiceRegisterNumber,
                ...
                $responseDepartments = wp_remote_post($this->taxApiUrl . '/configureDepartments', array(
                    'sslverify' => false,
                    'sslcertificates' => $savePath,
                    'headers' => ['Content-Type' => 'application/json; charset=utf-8'],
                    'body' => json_encode($departments, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
                ));

                if (is_wp_error($responseDepartments)) {
                    update_option('hkd_tax_service_api_departments_activated', 0);
                    ErrorService::hkdTaxServiceInsertTaxErrorReport(0, 'Tax service department request Error', $responseDepartments->get_error_message(), "no");
                    return false;
                }
                $bodyDepartments = json_decode($responseDepartments['body'], true);
            ...
            public function enqueueScripts($atgCodes)
            {
                ...
                wp_localize_script('hkd-tax-service-admin-main', 'my_ajax_object', array('ajax_url' => admin_url('admin-ajax.php')));
