...
if (!class_exists('ReportTable')) {
    class ReportTable extends \WP_List_Table
    {
        ...
        public function generate_qr_code($qr)
        {
            ...
            if($this->get_http_response_code('https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=' . str_replace(' ', '', $qr)) != "200"){
                if($this->get_http_response_code('https://quickchart.io/chart?cht=qr&chs=100x100&chl=' . str_replace(' ', '', $qr)) != "200"){
                    return 'https://qrcode.tec-it.com/API/QRCode?data=' . str_replace(' ', '', $qr);
                }else{
                    return 'https://quickchart.io/chart?cht=qr&chs=100x100&chl=' . str_replace(' ', '', $qr);
                }
            }else{
                return 'https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=' . str_replace(' ', '', $qr);
            }
        }
        ...
        public function deleteItems($ids)
        {
            ...
            $wpdb->query("DELETE FROM " . $wpdb->prefix . "tax_service WHERE ID IN($ids)");
        }
